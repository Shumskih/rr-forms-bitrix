...
if ($arParams["SHOW_STATUS"] == "Y") {
    ?>
    <p>
        <b><?= GetMessage("FORM_CURRENT_STATUS") ?></b>&nbsp;[<span
                class="<?= htmlspecialcharsbx($arResult["RESULT_STATUS_CSS"]) ?>"><?= htmlspecialcharsbx($arResult["RESULT_STATUS_TITLE"]) ?></span>]
    </p>
    <?
}
?>
                ...
                if (is_array($arQuestion['ANSWER_VALUE'])):
                    foreach ($arQuestion['ANSWER_VALUE'] as $key => $arAnswer) {
                        ...
                        <? if ($arAnswer["ANSWER_IMAGE"]): ?>
                            <? if (strlen($arAnswer["USER_TEXT"]) > 0): ?><?= $arAnswer["USER_TEXT"] ?><? endif ?>
                            <img src="<?= $arAnswer["ANSWER_IMAGE"]["URL"] ?>" <?= $arAnswer["ANSWER_IMAGE"]["ATTR"] ?>
                                 border="0"/>
                        <? elseif ($arAnswer["ANSWER_FILE"]): ?>
                            <a title="<?= GetMessage("FORM_VIEW_FILE") ?>" target="_blank"
                               href="<?= $arAnswer["ANSWER_FILE"]["URL"] ?>"><?= $arAnswer["ANSWER_FILE"]["NAME"] ?></a>
                            <br/>(<?= $arAnswer["ANSWER_FILE"]["SIZE_FORMATTED"] ?>)<br/>[&nbsp;<a
                                    title="<?= str_replace("#FILE_NAME#", $arAnswer["ANSWER_FILE"]["NAME"], GetMessage("FORM_DOWNLOAD_FILE")) ?>"
                                    href="<?= $arAnswer["ANSWER_FILE"]["URL"] ?>&action=download"><?= GetMessage("FORM_DOWNLOAD") ?></a>&nbsp;]
                        <? elseif (strlen($arAnswer["USER_TEXT"]) > 0): ?>
                            <?= $arAnswer["USER_TEXT"] ?>
                        <? else: ?>
                            <? if (strlen($arAnswer["ANSWER_TEXT"]) > 0): ?>
                                [<span class="form-anstext"><?= $arAnswer["ANSWER_TEXT"] ?></span>]
                                <? if (strlen($arAnswer["ANSWER_VALUE"]) > 0): ?>&nbsp;(<span
                                        class="form-ansvalue"><?= $arAnswer["ANSWER_VALUE"] ?></span>)<? endif ?>
                                <br/>
                            <? endif; ?>
                        <? endif; ?>
                        <?
